...
function Escreveai(letter) {
    if (currentGuess.length < WORD_LENGTH) {
        currentGuess += letter;
        let tileIndex = currentRow * WORD_LENGTH + currentGuess.length - 1;
        document.getElementById(`tile-${tileIndex}`).innerText = letter;
    }
}
...
function deleteLetter() {
    if (currentGuess.length > 0) {
        let tileIndex = currentRow * WORD_LENGTH + currentGuess.length - 1;
        document.getElementById(`tile-${tileIndex}`).innerText = "";
...
function checkGuess() {
    ...
    for (let i = 0; i < WORD_LENGTH; i++) {
        let tileIndex = currentRow * WORD_LENGTH + i;
